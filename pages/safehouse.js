...
class Safehouse extends React.Component {
	...
	continueToLogin = (values) => {
		...
		post(ApiPort.PostLogin, Postdata)
			...
			.catch((error) => {
				console.log("🚀 ~ file: Safehouse.js:119 ~ Safehouse ~ error:", error)
				this.setState({
	...
	successLogin(result, userName, isRedirect) {
		if (result?.accessToken?.accessToken && result.accessToken.refreshToken) {
			...
			localStorage.setItem('access_token', JSON.stringify('bearer ' + result.accessToken.accessToken));
			localStorage.setItem('refresh_token', JSON.stringify(result.accessToken.refreshToken));
			//获取会员资料
			// getMemberInfo((res) => {
			// 	this.hide();
			// 	Router.push('/');
	...
	render() {
		...
		console.log("🚀 ~ file: Safehouse.js:225 ~ Safehouse ~ render ~ status:", status)
		return (
			<Layout
				title="FUN88"
				Keywords=""
					...
					<div className="BoxOther sf">
						<div className="Box_Content">
							...
							<Form className="security-login-wrap" onSubmit={this.handleSubmit}>
											...
											},
											{
												validator: (rule, value, callback) => {
													if (value && !pwdReg.test(value)) {
														callback(translate('密码必须包含 6-20 个字母数字字符“A-Z”、“a-z”、“0-9”，并且可以包含 4 个特殊字符 ^# $@'));
													}
								...
								<Button size="large" type="primary" id="SubmitGeet" data-gt htmlType="submit" disabled={status}>
									{translate("提交")}
								</Button>
							</Form>
						</div>
