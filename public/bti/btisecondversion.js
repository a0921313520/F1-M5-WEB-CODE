...
if (strReferURL != "" && strReferURL != undefined) {
    ...
    SportsCookie.setCookie("ReferURLCooKie", strSSReferURL, 1);
}
...
(function () {
    ...
    var MemberToken = getUrlVars("MemberToken")
        ? getUrlVars("MemberToken").split("/")[0]
        : null;
    if (MemberToken != null) {
    ...
    var getStatusUrl = linkStatus;
    var getRefreshUrl = ReferURLLink;
    ...
    whl.prototype.status = function (callback) {
        ...
        if (strToken != "logout" && strToken != "") {
            ...
            xmlhttp.onreadystatechange = function () {
                if (xmlhttp.readyState == XMLHttpRequest.DONE) {
                    ...
                    if (xmlhttp.status == 200) {
                        var resultData = xmlhttp.response;
                        result.uid = new Date().getTime();
                        result.token = strToken;
                        result.status = "real";
                        result.balance = resultData.balance.toString();
            ...
            xmlhttp.setRequestHeader("Culture", "vi-vn");
