...
import {
    setFEWalletParams,
    getFEWalletParams,
} from "central-payment/platform.pay.config";
// import Withdrawal from "../../Central-Payment/Main/Web/M23/Withdrawal";
...
class WithdrawPage extends React.Component {
    constructor(props) {
        ...
        this.privateParams = {
            ...
            AddBankClick: () => {},
            HistoryClick: () => {
                this.navigationPage("records");
            },
            ApiGet: (url) => get(HostApi + url + apis),
            ApiPost: (url, postdata = "") =>
                post(HostApi + url + apis, postdata),
            goVerifyContact: () => {
                this.verifyContact();
            },
            goFinishKYCInfo: () => {
                this.navigationPage("uploadFiles");
            },
            goFinishMemberInfo: () => {
                console.log("goFinishMemberInfo-withdraw");
            },
            goDeposit: () => {
                this.props.goDeposit("deposit");
            },
            goAddBankInfo: (type = "", typekey = "", callback = () => {}) => {
                this.addBankCard(type, typekey, callback);
            },
            goRecord: () => {
                this.navigationPage("records");
            },
            ...
            modalTip: {
                info: (data = { closable: false }) => {
                    Modal.info({
                        ...
                        title: data?.title,
                        className:
                            "confirm-modal-of-public confirm-modal-of-forgetuser",
                        ...
                        onOk: () => {
                            data?.okFunction &&
                                typeof data.okFunction === "function" &&
                                data.okFunction();
                        },
                        onCancel: () => {
                            data?.noFunction &&
                                typeof data.noFunction === "function" &&
                                data.noFunction();
                        },
                    });
                },
                confirm: (data = {}) => {
                    const textAlignStyle =
                        data?.children?.type?.name === "PhoneEmailVerification"
                            ? "left"
                            : "center";
                    Modal.confirm({
                        icon: null,
    ...
    componentDidUpdate(prevProps, prevState) {
        if (
            ...
            this.props.setWithdrawList(
                this.state.withdrawMethods,
                this.state.currentWithdrawKey,
            );
        }

        if (
            prevProps.currentWithdrawKey !== this.props.currentWithdrawKey &&
            this.props.currentWithdrawKey
        ) {
    ...
    navigationPage = (key) => {
        switch (key) {
            case "records":
                if (
                    ~global.location.pathname.indexOf("/vn/transaction-record")
                ) {
    ...
    handleOk() {
        ...
        AddExchangeRateWallets(datas, (res) => {
            ...
            if (res.isSuccess == true) {
                ...
                typeof this.tdbfunction === "function" &&
                    this.tdbfunction(this.state.ctcMethod);
                this.setState({
                    tDBName: "",
                    tDBAddress: "",
                    addTDBBtnStatue: false,
                ...
                if (res.walletAddressErrorList.length) {
                    message.error(res.walletAddressErrorList[0].description);
    ...
    addExchangeRateWallets = (type, typekey, callback) => {
        console.log(
            "🚀 ~ file: WithdrawPage.js:274 ~ WithdrawPage ~ addExchangeRateWallets ~ type,typekey,callbac:",
            type,
            typekey,
            callback,
        );
    ...
    changeModalBtnStatus() {
        const { tDBNameError, tDBAddressError, tDBAddress, tDBName } =
            this.state;
            ...
            .then((res) => {
                if (
                    ...
                    type &&
                        ...
                        this.addExchangeRateWallets(type, typekey, callback);
                    //短信和语言切换查询完次数后 call
    ...
    verifyContact = () => {
        ...
        console.log(
            "🚀 ~ file: WithdrawPage.js:360 ~ WithdrawPage ~ memberInfo:",
            memberInfo,
        );
    ...
    render() {
        ...
        console.log(
            "🚀 ~ file: WithdrawPage.js:405 ~ WithdrawPage ~ render ~ checkAbleSmsOTP:",
            checkAbleSmsOTP,
        );
        const { memberInfo } = this.props;
        return (
            <React.Fragment>
                ...
                <Withdrawal
                    ...
                    setPayMethods={(list) => {
                        this.setState({ withdrawMethods: list });
                    }}
                    setPayMethodChange={(run) => {
                        this.payMethodChange = run;
                    }}
                />
                ...
                <Announcement optionType="Withdrawal" />
                ...
                {/* 提款验证（邮箱和电话） */}
                {toAccountVerification && withdrawalVerifyStep && (
                    <AccountVerification
                ...
                {showAddCard && (
                    <AddBankCard
                        ...
                        alreadyBindBanks={[]}
                        getMemberBanksList={
                            this.props.GetDepositMemberBanks &&
                            this.props.GetDepositMemberBanks()
                        }
                    />
                )}
