...
class DepositPage extends React.PureComponent {
    constructor(props) {
        ...
        console.log(
            "🚀 ~ DepositPage ~ constructor ~ dialogTabKey:",
            dialogTabKey,
        );
        const bonusId =
            (dialogTabKey?.bonusId && String(dialogTabKey?.bonusId)) || "";
        ...
        this.privateParams = {
            ...
            AddBankClick: (type = "deposit", category = "bank") => {
                this.addBankCardModal();
            }, //deposit存款, bank是普通银行，其他是虚拟币钱包,添加银行成功后去更新银行call this.props.GetDepositMemberBanks && this.props.GetDepositMemberBanks()
            HistoryClick: (type = "Deposit") => {
                this.navigationPage("records");
            }, //type = Deposit存款记录,Withdrawals提款记录
            goRecord: (type = "Deposit", method) => {
                console.log("method:", method), this.navigationPage("records");
            },
            ApiGet: (url) => {
                let host = HostApi;
    ...
    componentDidMount() {
        ...
        if (!!sessionStorage.getItem("promoDeposit")) {
            const {
                ...
                bonusTitle = "",
                bonusProduct = "",
            } = JSON.parse(sessionStorage.getItem("promoDeposit")) || {};
            this.privateParams = {
                ...this.privateParams,
                bonusId: bonusId,
    ...
    render() {
        const { nameModalVisible, name, showAddCard, nameModalLoad } =
            this.state;
        return (
            <React.Fragment>
                <Loading />
                <div>{this.state.show && <Deposit />}</div>
                <Modal
                ...
                {/* 紧急公告弹窗 */}
                <Announcement optionType="Deposit" />
                {showAddCard && (
                    <AddBankCard
                        visible={showAddCard}
                        closeModal={() => {
                ...
                <Modal
                    ...
                    maskClosable={true}
                    onCancel={() => {
                        this.setState({ phcLessonModal: false });
                    }}
