...
function OtpPopUp({
        ...
        const fetchData = async () => {
            try {
                ...
                const [res1,res2] = await Promise.all([
                    get(ApiPort.VerificationAttempt + `&channelType=Email&serviceAction=${action}`),
                    get(ApiPort.VerificationAttempt + `&channelType=SMS&serviceAction=${action}`),
                ])
                ...
                console.error('Error VerificationAttempt 获取用户验证次数:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const VerifyCategory = (type, otpVisible, status) => {
        console.log("🚀 ~ VerifyCategory ~ type, otpVisible, status:", type, otpVisible, status)
        if (type == "Phone") {
                    ...
                    <React.Fragment>
                        <Row>
                            ...
                            <div className="OtpList">
                                {memberInfo &&  Array.isArray(verifyList) &&
                                    verifyList.length &&
                            ...
                            {(Array.isArray(verifyList) && verifyList.length === 0) && (
                                <div className="OtpList">
