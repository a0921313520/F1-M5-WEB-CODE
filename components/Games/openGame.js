...
class openGame extends React.Component {
    ...
    openGame(items) {
        ...
        if (
            ...
            const routerPath = GameStatic.find((game)=>game.providerName === items.gameCatCode).path;
            if(routerPath){
    ...
    actionGame(items, isTriggerFromGameLobbyButton) {
        const IS_THIS_ALLOW_GUEST_OPEN_GAME = isThisAllowGuestOpenGame(items.gameCatCode, items.provider);
        ...
        Router.push(
            `/games/${routerPath}/${items.provider}/?gameid=${items.gameId}&vendor=${items.gameCatCode}&name=${items.provider}&demo=${isGuest() ? 'true' : 'false'}&launchGameCode=${items.launchGameCode || ""}`+ queryOfIsTriggerFromGameLobbyButton
        );
    }
    ...
    */
    async checkBalanceNeedAsyncAPIChecking(code, provider) {
        ...
        const NEED_SETUP_BALANCE_FROM_API_THEN_showBalanceError = isEnterGameByJustRefreshGameUrl && isBalanceDoesNotSetup;
    ...
    checkBalance(code, provider) {
        ...
        switch (code) {
            ...
            case "Slot":
                if (provider == "IMOPT") {
                    const status4pttiger = provider == "IMOPT" && window.IMOPT < 20;
        ...
        get(
            ...
            if (res.result && res.result.gameDetails.length != 0) {
                ...
                if (item) {
                    ...
                    Router.push(
                        `/games/arcade-games/${item.provider}/?gameid=${item.gameId}&name=${item.provider}&type=&demo=${false}&launchGameCode=${item.launchGameCode || ""}` + queryOfIsTriggerFromGameLobbyButton
                    );
                } else {
                    Modal.info({
            ...
            <React.Fragment>
                ...
                >
                    ...
                    <div className="buttonWrap">
                        ...
                        >
                            <span className="ticket743uat">{translate("返回大厅")}</span>
                        </Button>
                        <Button
                            onClick={() => {
                                this.setState(
                        ...
                        >
                            <span className="ticket743uat">{translate("银行(大写)")}</span>
                        </Button>
                    </div>
                </Modal>
            </React.Fragment>
