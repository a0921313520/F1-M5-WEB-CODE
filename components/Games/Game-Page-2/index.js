...
import {
    staticHeaderGtag,
    gameListPageTrackingPiwik,
    gameListOpenGamePiwik,
    gameListPageCheckMorePiwik,
...
class Home extends React.Component {
    ...
    changeGamesType(Recommended) {
        ...
        get(
            ...
            if (
                res &&
                res.result &&
                Array.isArray(res.result.gameDetails) &&
                res.result.gameDetails.length
            ) {
                const gameDetails = res.result.gameDetails;
                const jackpotGameDetails = gameDetails.filter((item) =>
                    item.categories.some(
                        (cate) => cate.categoryName === "Jackpot",
                    ),
                );
    ...
    render() {
        const { isLoading, isSlot } = this.state;
        const { GameInfo, Routertype } = this.props;
        ...
        return (
            <Spin spinning={isLoading} tip={translate("加载中")}>
                <div className="common-distance">
                    <GameBanner Routertype={Routertype} />
                    <div className="Gameslobby game-box-content">
                        ...
                        >
                            ...
                            {this.state.SubCategory &&
                                !this.dontShowGameTypeList.includes(
                                    Routertype,
                                ) && (
                                    <React.Fragment>
                                        <h3 className="Title">
