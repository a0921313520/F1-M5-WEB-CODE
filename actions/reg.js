...
//门牌号和街道名称
export const address2Reg =
    /^[A-Za-z0-9'\u00e0-\u1EF4\#\'\.\,\。\，\-\/\\&\()\s]{5,100}$/;

//身份证  18位
export const idCard = /^[0-9]\d{16}(\d|x|X)$/;

// 城市/城镇格式
export const cityReg =
...
// 申请优惠表单备注字段格式
export const remarkReg =
    /^[0-9a-zA-ZàáãạảăắằẳẵặâấầẩẫậèéẹẻẽêềếểễệđìíĩỉịòóõọỏôốồổỗộơớờởỡợùúũụủưứừửữựỳỵỷỹýÀÁÃẠẢĂẮẰẲẴẶÂẤẦẨẪẬÈÉẸẺẼÊỀẾỂỄỆĐÌÍĨỈỊÒÓÕỌỎÔỐỒỔỖỘƠỚỜỞỠỢÙÚŨỤỦƯỨỪỬỮỰỲỴỶỸÝ\,\.\:\!\?\-\)\(\/\&\#\s ]{1,150}$/;

//USDT钱包名
export const usdtWalleNameReg =
    /^[0-9a-zA-Z_ƢƣÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêếìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹý\s]{1,20}$/;
...
//是否包含字符或空格
export const specialCharactersReg =
    /[!@#$%^&*()_+-={}/|\[\]:;'"<>,.?~\s、 \\。·]*$/;
